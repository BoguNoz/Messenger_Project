...
namespace Messager_Project.Repository.MessageEmote
{
    ...
    {
        public MSMessageEmotesRepository(DbContext dbContext) : base(dbContext)
        {
        }
        ...
        public async Task<ResponseModel<MessageEmotes>> SaveRelationshipAsync(MessageEmotes relation, Emotes emote, Message message)
        {
            ...
            if (emote == null)
                return new ResponseModel<MessageEmotes> { Status = false, Message = "EmoteRepositories is null", ReferenceObject = null };
            if (message == null)
                return new ResponseModel<MessageEmotes> { Status = false, Message = "MessageRepositories is null", ReferenceObject = null };
